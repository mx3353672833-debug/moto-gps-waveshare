...
The functional prototype is feasible; the H0175 EVT A1 has produced a complete schematic, four-layer routing and review manufacturing files from the frozen R4 routing baseline, but the first board still has to close the FPC, USB-C, power, RF, procurement and complete-product mechanical gates first. A pure Safari/PWA cannot deliver continuous positioning after the iPhone locks, so the formal path must keep the terminal's standalone GNSS.
...
| H0175 EVT A1 main board | Medium-high | Derived from the frozen R4 routing; ERC/DRC/unconnected/consistency all 0; physical gates not cleared |
...
| Compass | Vehicle heading rather than the phone direction | GNSS + QMI8658A + IIS2MDC | Component ID, axes, fusion and complete-product calibration await real hardware |
...
- The IIS2MDC must first pass component ID, self-test and board-level axis regression, and then be soft-iron / hard-iron calibrated on the final PCB, enclosure, Garmin mount, handlebar and a powered motorcycle.
...
![In-house hardware overview](assets/pcb-a1-top.png)
...
| Display / touch | H0175Y003AMT003 V1 / CST820 | 1.75 in 466×466 fully laminated AMOLED; CO5300 QSPI; single-point touch; on A1 it is powered directly by 3V3_DISPLAY |
...
| IMU / compass | QMI8658A C3021082 + IIS2MDCTR C2655002 | EVT controlled direct substitutes; frozen after ID/interrupt/axis/complete-product calibration |
| Buttons | EVQP7C01P C388883 | Actuator 1.1 ± 0.1 mm; the enclosure button post travel is frozen against the physical part |
...
The H0175 EVT A1 has been derived from the frozen R4 routing baseline, with the current display, sensors, buttons and procurement fields synchronised; the power test coupon, controlled impedance and complete-product RF/thermal verification are still board-release gates.

### Power path

USB-C 5 V → TUSB320LAI → BQ25628E ↔ single-cell lithium battery/NTC → TPS63070 3.3 V; the GNSS can be powered separately from a low-noise 3.0 V LDO. On page 7 of the controlled PDF the H0175 EVT A1 supplies TP3.3/IOVCC/VCI/VBAT/VCI_EN directly from 3V3_DISPLAY, and the first board must be verified for cold start, low temperature and maximum brightness.

### Interfaces and buttons

The official MPN of J1 is U262-161N-4BVC11, while the historical KiCad footprint name still contains 16XN; SW1–SW4 are EVQP7C01P (C388883), and the 1.1 ± 0.1 mm actuator means the button post travel must be frozen with the part mounted in the enclosure.
...
| --- | --- |
| A1 CAD | Derived from the R4 routing baseline: 93 production components, 458 pads, 1555 trace segments, 189 vias, 2 copper pours |
...
> KiCad 10.0.6 full-board check results: ERC 0, DRC 0, unconnected 0, schematic/PCB consistency errors 0. In the same state the H0175 EVT A1 exported Gerbers, drill files, a grouped BOM and a CPL; the files are still marked NOT FOR FABRICATION because the physical, power, RF, stock or sourcing release states are not all closed.
...
| In-house main board | Medium-high | H0175 EVT A1 has a complete schematic/four-layer routing; full-board DRC and consistency both 0 | The first board can be ordered only after the physical interface and a power/RF review |
...
| Mass production | Not yet in place | The H0175 EVT A1 is a reviewable electrical candidate, not a production release | First-board verification → EVT → Rev B → DVT |

> Overall judgement: the schematic and PCB CAD stage is complete; only after the FPC/USB-C physical parts, the footprint review, the power coupon, RF and the procurement states are closed is the formal package for ordering the first batch of 5 PCBAs generated. The current H0175 EVT A1 review Gerbers must not be ordered directly.
...
| --- | --- | --- |
| Display/FFC/power incompatibility | The board cannot light up, the touch direction is wrong, or high-brightness start-up fails | Fix on the H0175Y003AMT003 V1; the first sample's continuity check confirms Pin 1, the contact faces at both ends and the end-to-end order; test 3V3_DISPLAY direct supply for cold start, low temperature and maximum brightness |
...
| 4 Mechanical environment | 20 fitting cycles, Garmin mount, spray, thermal cycling, vibration, drop, USB port cover, EVQP7C01P buttons | Looseness before/after / leakage / 1.1 mm actuator travel re-check |
...
| P3 Rev A design | Six-volume schematic, four-layer layout, BOM, Gerber | The H0175 EVT A1 has reached ERC/DRC/unconnected/consistency all 0 |
...
The H0175 EVT A1 is now a complete, openable, reviewable electrical engineering candidate; it is derived from the frozen R4 routing and passes all KiCad connectivity and rule checks, but it still cannot be ordered directly until the physical and verification gates are closed.

![Current completion state and next steps](assets/v3-isometric.png)

| State | Done / to be done |
| --- | --- |
| Done | The shared NavCore, RouteBundle, Web round display, backend gateway, V3 CAD/STEP/2D assembly drawing/Garmin A0 mount, plus the H0175 EVT A1 complete schematic, four-layer PCB, BOM, CPL and review Gerber/drill files |
| Passed | KiCad 10.0.6: ERC 0, full-board DRC 0, unconnected 0, consistency 0; the BOM/CPL 93 placed references fully reconciled; the V3 STEP solid is valid |
| Awaiting physical parts | Long testing with a real AMap key, continuity of the H0175Y003AMT003 V1 display and the 31P FFC plus 3V3_DISPLAY start-up/brightness, QMI8658A firmware regression, IIS2MDC complete-product calibration, EVQP7C01P button travel, GNSS antenna, hotspot, Garmin gauge, temperature rise, sealing, vibration |
