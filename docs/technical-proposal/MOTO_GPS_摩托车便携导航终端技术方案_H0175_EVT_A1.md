> **语言 / Language:** 中文 · [English](MOTO_GPS_Portable_Navigation_Terminal_Technical_Proposal_H0175_EVT_A1.en.md)

# MOTO GPS 摩托车便携导航终端技术方案（H0175 EVT A1）

版本：2026-09-03  ·  状态：功能原型 / H0175 EVT A1 电气工程候选  ·  NOT FOR FABRICATION

> **历史冻结交付文档，不代表当前软件样机。** 本文保留 H0175 EVT A1 的“独立 GNSS + 手机热点”假设，用于追溯硬件候选，禁止据此执行当前联调或下单。当前基线是 Waveshare 1.75C + 原生 iOS App：手机负责真实定位、按当前位置优先的地点搜索和高德路线，ESP32 通过 BLE 显示；App 测试版及固件导航页长按均有明确标注的演示入口，生产导航仍使用真实路线，iOS BLE 恢复握手/重连的已知实现问题已修复。见 [微雪 + iPhone 样机基线](../WAVESHARE_IOS_PROTOTYPE.md)。配套 PDF/DOCX 是同一历史归档，未随当前软件基线重写。
...
功能原型可行；H0175 EVT A1 已从冻结 R4 路由基线形成完整原理图、四层布线和审阅制造文件，但首板仍必须先关闭 FPC、USB-C、电源、RF、采购和整机结构门槛。纯 Safari/PWA 无法满足 iPhone 锁屏后持续定位，因此正式路径必须保留终端独立 GNSS。
...
![自研硬件总体方案](assets/pcb-a1-top.png)
...
USB-C 5 V → TUSB320LAI → BQ25628E ↔ 单节锂电池/NTC → TPS63070 3.3 V；GNSS 可由低噪声 3.0 V LDO 单独供电。H0175 EVT A1 按受控 PDF 第 7 页由 3V3_DISPLAY 直接给 TP3.3/IOVCC/VCI/VBAT/VCI_EN 供电，首板必须做冷启动、低温和最大亮度验证。

### 接口与按键

J1 官方 MPN 为 U262-161N-4BVC11，KiCad footprint 历史名仍含 16XN；SW1–SW4 为 EVQP7C01P（C388883），1.1 ± 0.1 mm 执行部必须用装壳实物冻结按键柱行程。
...
> KiCad 10.0.6 全轨检查结果：ERC 0、DRC 0、未连接 0、原理图/PCB 一致性错误 0。H0175 EVT A1 同状态导出了 Gerber、钻孔、分组 BOM 和 CPL；文件仍标记 NOT FOR FABRICATION，因为实物、电源、RF、库存或代采放行状态尚未全部关闭。
...
> 总体判断：原理图和 PCB CAD 阶段已经完成；关闭 FPC/USB-C 实物、封装复核、电源券、RF 与采购状态后，才生成可下单首批 5 块 PCBA 的正式包。当前 H0175 EVT A1 审阅 Gerber 禁止直接下单。
